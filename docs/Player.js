export class Player {
    ...
    constructor(game) {
        ...
        this.hp = this.maxHP;
        this.maxAmmo = 10;
        this.ammo =this.maxAmmo ; // 残弾数
        this.ammoRecoveryTime = 3; // n秒で1回復（例: 1秒）
        this.ammoRecoveryTimer = 0; // 経過時間（秒）
        this.attackRadius = 50; // 攻撃範囲の半径（ピクセル）
        
    }

    update(deltaTime) {
        if (this.game.keys['w']) this.y -= Player.SPEED;
        if (this.game.keys['s']) this.y += Player.SPEED;
        if (this.game.keys['a']) this.x -= Player.SPEED;
        if (this.game.keys['d']) this.x += Player.SPEED;
